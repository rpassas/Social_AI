...
class Agent_Semi_Bayes():
    ...
    def __init__(self, state_size=3, seed=None, memory=0, behav_control=0, model_var=1, behav_initial_spread=1, pred_initial_spread=1, inference_fn='IRL',  action_cost_fn='linear'):
        ...
        self.alpha_pred = np.random.randint(2, high=3, size=self.state_size)
        self.beta_pred = np.random.randint(2, high=3, size=self.state_size)
        # for estimating the expectations of others
        self.alpha_conf = np.random.randint(2, high=3, size=self.state_size)
        self.beta_conf = np.random.randint(2, high=3, size=self.state_size)
        ...
        # these do nothing in this agent
    ...
    def learn_predict_world(self):
        ...
        '''
        self.alpha_pred = [x + y for x,
                           y in zip(self.alpha_pred, self.world[-1])]
        self.beta_pred = [x - y + 1 for x,
                          y in zip(self.beta_pred, self.world[-1])]
        prior_dist = [stats.beta.cdf(x=self.possible_priors+.001, a=self.alpha_pred[i], b=self.beta_pred[i]) - stats.beta.cdf(
            x=self.possible_priors, a=self.alpha_pred[i], b=self.beta_pred[i]) for i in range(self.state_size)]
        likelihood_dist = [stats.binom.pmf(
            k=self.alpha_pred[i], n=len(self.world)+2, p=self.possible_priors) for i in range(self.state_size)]
